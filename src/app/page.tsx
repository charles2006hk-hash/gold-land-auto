...
const CrossBorderView = ({ 
        ...
        Object.entries(dateFields).forEach(([fieldKey, label]) => {
            ...
            if (dateStr) {
                ...
                if (days !== null) {
                    ...
                    if (days < 0) expiredItems.push(itemData); else if (days <= 30) soonItems.push(itemData);
                }
            }
        });
    });
    ...
    const convertDateToTask = (dateKey: string, dateLabel: string, dateVal: string) => {
        ...
        if(confirm(`確定要為「${dateLabel}」建立一筆收費紀錄嗎？\n\n日期: ${dateVal}\n預設費用: $${defaults.fee || 0}`)) {
            const newTask: CrossBorderTask = { id: Date.now().toString(), date: new Date().toISOString().split('T')[0], item: `代辦${dateLabel}`, fee: Number(defaults.fee) || 0, days: defaults.days, institution: '公司', handler: '', currency: 'HKD', note: `來自日期提醒 (${dateVal})`, isPaid: false };
            ...
            {reportModalData && (
                <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={() => setReportModalData(null)}>
                    <div className="bg-white w-full max-w-3xl rounded-xl shadow-2xl flex flex-col max-h-[90%]" onClick={e => e.stopPropagation()}>
                        <div className={`p-4 text-white flex justify-between items-center ${reportModalData.type === 'expired' ? 'bg-red-800' : 'bg-amber-700'}`}><h3 className="font-bold text-lg">{reportModalData.title}</h3><button onClick={() => setReportModalData(null)}><X/></button></div>
                        ...
                        <>
                            ...
                            <div className="p-4 border-b overflow-x-auto whitespace-nowrap flex gap-3 bg-slate-50/30 flex-none pb-2 scrollbar-hide">
                                {Object.entries(dateFields).map(([key, label]) => {
                                    ...
                                    return (
                                        <div key={key} className={`inline-block p-2 rounded-lg border text-center min-w-[100px] ${color} group relative snap-center`}>
                                            ...
                                            <button onClick={(e) => { e.stopPropagation(); convertDateToTask(key, label, dateVal); }} className="absolute inset-0 bg-blue-600/90 text-white text-[10px] font-bold opacity-0 group-hover:opacity-100 flex items-center justify-center rounded-lg transition-opacity"><DollarSign size={12} className="mr-1"/> 轉收費</button>
                                        </div>
                                    );
                                })}
                            </div>
...
// ------------------------------------------------------------------
// ★★★ 5. Settings Manager (v9.6: 完整無刪減終極版) ★★★
// 包含：所有舊設定功能 + 用戶密碼 + 系統日誌 + 備份救援 + 數據透視
// ------------------------------------------------------------------
const SettingsManager = ({ 
    ...
    // 6. Database Categories States
    const [selectedDbCat, setSelectedDbCat] = useState('Person');
    const [newDocType, setNewDocType] = useState('');

    // 7. System Logs States
    const [logs, setLogs] = useState<any[]>([]);

    // 8. Data Inspector State
    ...
    const handleUnlockInspector = () => {
        ...
        if (!pwd) return;
        if (staffId === 'BOSS' && pwd === '8888') { setShowInspector(true); return; }
        const currentUserObj = systemUsers.find(u => u.email.toLowerCase() === staffId.toLowerCase());
        if (currentUserObj && currentUserObj.password === pwd) { setShowInspector(true); } 
    ...
    const updateUsersDb = async (newList: any[]) => { if (db) await setDoc(doc(db, 'artifacts', appId, 'staff', 'CHARLES_data', 'system', 'users'), { list: newList }, { merge: true }); };
    ...
    const handleCloudBackup = async (silent = false) => {
        if (!storage || !appId) { if (!silent) alert("錯誤：無法連接雲端儲存空間"); return; }
        setIsBackingUp(true);
        try {
                ...
                {activeTab === 'expenses' && (
                    <div className="space-y-6">
                        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
                            ...
                            <div className="flex flex-wrap gap-2 mt-3">{(settings.paymentTypes || []).map((pt, i) => (<span key={i} className="bg-slate-100 px-2 py-1 rounded text-xs flex items-center gap-2 border border-slate-200 group">{pt} <button onClick={() => {setPayTypeInput(settings.paymentTypes[i]); setEditingPayTypeIndex(i);}} className="text-slate-400 hover:text-blue-600"><Edit size={10}/></button><button onClick={() => { const list = [...settings.paymentTypes]; list.splice(i, 1); updateSettings('paymentTypes', list); }} className="text-slate-400 hover:text-red-500"><X size={10}/></button></span>))}</div>
                        </div>
                        <div className="bg-white p-5 rounded-xl border border-slate-200 shadow-sm">
...
const DatabaseSelector = ({ 
  ...
  // 1. Vehicle Form Modal (v9.91: 費用自動連結中港業務 + 完整功能)
const VehicleFormModal = ({ 
    ...
    // 4. 總支出 (一般雜費 + 中港代辦成本)
    // 這裡假設代辦費也是成本的一部分 (Cost of Goods Sold)
                 ...
                 {showVrdOverlay && (
                    <div className="absolute inset-0 z-30 bg-white/95 backdrop-blur-sm flex flex-col p-6 animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="flex flex-col h-full overflow-hidden pb-10">
                            ...
                            <div className="flex-1 overflow-y-auto space-y-2 pr-1">{vrdResults.length > 0 ? (vrdResults.map((res, idx) => (<div key={idx} className="bg-blue-50 border border-blue-200 p-3 rounded-xl flex justify-between items-center hover:bg-blue-100 transition-colors"><div><div className="font-bold text-lg text-slate-800">{res.plateNoHK || res.regNo}</div><div className="text-xs text-slate-600">{res.manufactureYear} {res.make} {res.model}</div><div className="text-[10px] text-slate-400 font-mono">Chassis: {res.chassisNo}</div></div><button type="button" onClick={() => applyVrdData(res)} className="bg-green-500 text-white px-4 py-2 rounded-lg font-bold hover:bg-green-600 shadow-sm text-xs">導入此車</button></div>))) : (!searching && vrdSearch && <div className="text-center text-slate-400 mt-10">無相符資料</div>)}</div>
                        </div>
                    </div>
                 )}
                        ...
                        <div className="p-4 space-y-3">
                            ...
                            <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
                                <div className="col-span-1">
                                    <label className="text-[9px] text-slate-400 font-bold uppercase">Year</label>
                                    <input name="year" type="number" defaultValue={v.year} className="w-full bg-slate-50 border-b border-slate-200 p-1 text-sm font-mono"/>
                                </div>
                                <div className="col-span-1">
                                    <label className="text-[9px] text-slate-400 font-bold uppercase">Mileage</label>
                                    <input name="mileage" value={mileageStr} onChange={(e) => setMileageStr(formatNumberInput(e.target.value))} className="w-full bg-slate-50 border-b border-slate-200 p-1 text-sm font-mono text-right" placeholder="km"/>
                                </div>
                                <div className="col-span-1">
                                    <label className="text-[9px] text-slate-400 font-bold uppercase">Prev Owners</label>
                                    <input name="previousOwners" defaultValue={v.previousOwners} className="w-full bg-slate-50 border-b border-slate-200 p-1 text-sm text-right"/>
                                </div>
                                {/* ★★★ 這裡就是補回的輸入框 ★★★ */}
                                <div className="col-span-1 bg-red-50/50 rounded px-1">
                                    <label className="text-[9px] text-red-400 font-bold uppercase">Lic. Expiry (牌費)</label>
                                ...
                                <div className="space-y-1 mb-2">
                                    ...
                                    {(v.expenses || []).map((exp: any) => (
                                        <div key={exp.id} className="grid grid-cols-12 gap-2 text-xs p-1.5 bg-white border rounded shadow-sm items-center">
                                            ...
                                            <div className="col-span-1 text-right"><button type="button" onClick={() => deleteExpense(v.id!, exp.id)} className="text-red-400 hover:text-red-600"><X size={12}/></button></div>
                                        </div>
                                    ))}
                                    {/* 2. 中港業務費用 (自動連結) */}
                                    {(v.crossBorder?.tasks || []).filter((t:any) => t.fee > 0).map((task: any) => (
                                        <div key={`cb-${task.id}`} className="grid grid-cols-12 gap-2 text-xs p-1.5 bg-blue-50/50 border border-blue-100 rounded shadow-sm items-center text-blue-800">
                                            <div className="col-span-2 text-blue-400">{task.date}</div>
                                            <div className="col-span-3 font-bold flex items-center"><Globe size={10} className="mr-1"/> {task.item}</div>
                                            <div className="col-span-3 text-blue-600/70 truncate">中港業務</div>
                                            <div className="col-span-3 font-mono text-right">{formatCurrency(task.fee)}</div>
  ...
  // 2. Report View (v9.94: 應收報表增強 - 含日期與類別篩選)
  const ReportView = () => {
    // 新增類別篩選狀態
    ...
    const generateReportData = () => {
        ...
        if (reportType === 'receivable') {
            // 1. 先進行資料轉換與計算
            const rawData = inventory.map(v => {
                const cbFees = (v.crossBorder?.tasks || []).reduce((sum, t) => sum + (t.fee || 0), 0);
                const totalReceivable = (v.price || 0) + cbFees;
                const received = (v.payments || []).reduce((s, p) => s + (p.amount || 0), 0);
                const balance = totalReceivable - received;
                
                // 決定顯示日期：出庫日 > 第一個任務日 > 入庫日
                const displayDate = v.stockOutDate || (v.crossBorder?.tasks?.[0]?.date) || v.stockInDate || '';
                
                // 決定類別：如果是已售/已訂且有車價，算車輛銷售；否則算純代辦
                const type = (v.status === 'Sold' || v.status === 'Reserved') && (v.price && v.price > 0) ? 'Vehicle' : 'Service';
                ...
                return {
                    ...
                    title: `${v.year} ${v.make} ${v.model}`,
                    regMark: v.regMark,
                    amount: balance, // 這裡是欠款餘額
                    status: v.status,
                    type: type // 用於篩選
                };
            });

            // 2. 進行過濾 (欠款 > 0  &&  日期符合  &&  類別符合)
            data = rawData.filter(item => {
                // A. 必須有欠款
                if (item.amount <= 0) return false;

                // B. 日期過濾 (如果有選日期)
                const isDateMatch = (!reportStartDate || !reportEndDate) || 
                                    (item.date >= reportStartDate && item.date <= reportEndDate);
            ...
            if (reportStartDate && reportEndDate) {
                data = data.filter(d => d.date >= reportStartDate && d.date <= reportEndDate);
            }
    ...
    return (
        <div className="p-6 bg-white rounded-lg shadow-sm min-h-screen">
            ...
            {/* 控制面板 */}
            <div className="bg-gray-50 p-4 rounded border mb-6 print:hidden grid grid-cols-1 md:grid-cols-5 gap-4">
                <div className="col-span-1">
                    <label className="block text-xs font-bold text-gray-500 mb-1">報表類型</label>
                    <select value={reportType} onChange={e => setReportType(e.target.value as any)} className="w-full border p-2 rounded text-sm">
                ...
                {reportType === 'receivable' && (
                    <div className="col-span-1">
                        ...
                        <select value={reportCategory} onChange={e => setReportCategory(e.target.value as any)} className="w-full border p-2 rounded text-sm">
                            <option value="All">全部 (All)</option>
                            <option value="Vehicle">車輛銷售 (Sales)</option>
                            <option value="Service">代辦服務 (Services)</option>
                        </select>
                    </div>
                )}
                ...
                <div className="col-span-1">
                    ...
                    <input type="date" value={reportStartDate} onChange={e => setReportStartDate(e.target.value)} className="w-full border p-2 rounded text-sm" />
                </div>
                <div className="col-span-1">
                    <label className="block text-xs font-bold text-gray-500 mb-1">結束日期</label>
                    <input type="date" value={reportEndDate} onChange={e => setReportEndDate(e.target.value)} className="w-full border p-2 rounded text-sm" />
                </div>

                {reportType === 'payable' && (
                    <div className="col-span-1">
                        <label className="block text-xs font-bold text-gray-500 mb-1">負責公司</label>
                        <select value={reportCompany} onChange={e => setReportCompany(e.target.value)} className="w-full border p-2 rounded text-sm"><option value="">全部公司</option>{settings.expenseCompanies?.map(c => <option key={c} value={c}>{c}</option>)}</select>
                    </div>
                )}
            </div>
            ...
            <div className="print:visible">
                <div className="text-center mb-8 hidden print:block">
                    ...
                    <h2 className="text-xl font-bold border-b-2 border-black inline-block pb-1 mb-2">
                        {reportType === 'receivable' ? '應收未收報表 (Accounts Receivable)' : reportType === 'payable' ? '應付未付報表 (Accounts Payable)' : '銷售數據統計 (Sales Report)'}
                    </h2>
                            ...
                            <tr key={idx} className="border-b hover:bg-yellow-50 cursor-pointer print:cursor-auto print:hover:bg-transparent" onClick={() => handleReportItemClick(item.vehicleId)}>
                                ...
                                {reportType === 'receivable' && (
                                    <td className="p-2 border text-xs">
                                        <span className={`px-2 py-0.5 rounded border ${item.type==='Vehicle'?'bg-blue-50 text-blue-700 border-blue-100':'bg-purple-50 text-purple-700 border-purple-100'}`}>
                                            {item.type === 'Vehicle' ? '銷售' : '服務'}
                                        </span>
                                    </td>
                                )}
      ...
      <main className="flex-1 w-full min-w-0 md:ml-0 p-4 md:p-8 print:m-0 print:p-0 transition-all duration-300 flex flex-col h-screen overflow-hidden">
        ...
        <div className={`${isPreviewMode ? 'hidden' : 'block'} ${activeTab === 'reports' ? 'print:block' : 'print:hidden'} flex flex-col h-full overflow-hidden`}>
         ...
         {/* Cross Border Tab (v10.5: TypeScript 類型修復) */}
          ...
          {/* Dashboard Tab (v10.6: 修復狀態顯示、補全欄位、手機隱藏提醒) */}
          {activeTab === 'dashboard' && (
            <div className="flex flex-col h-full overflow-hidden space-y-4 animate-fade-in relative">
              ...
              {/* 2. 提醒中心 (★ 修正：手機版隱藏 hidden md:grid ★) */}
              {(() => {
                  // ... (保留原本的 docAlerts, cbAlerts 邏輯) ...
                  const docAlerts: any[] = [];
                  ...
                  inventory.filter(v => v.crossBorder?.isEnabled).forEach(v => { Object.entries(cbDateFields).forEach(([field, label]) => { const dateStr = (v.crossBorder as any)?.[field]; if (dateStr) { const days = getDaysRemaining(dateStr); if (days !== null && days <= 30) cbAlerts.push({ id: v.id, title: v.regMark || '未出牌', desc: label, date: dateStr, days, status: days < 0 ? 'expired' : 'soon', raw: v }); } }); });
                          ...
                          <div className="overflow-y-auto p-2 space-y-1.5 scrollbar-thin scrollbar-thumb-white/20">
                              {items.map((item:any, idx:number) => (
                                  <div key={idx} onClick={() => onItemClick(item)} className={`flex justify-between items-center p-2 rounded text-xs cursor-pointer hover:bg-white/10 border-l-2 ${item.status === 'expired' ? 'border-red-500 bg-red-900/10' : 'border-amber-400 bg-amber-900/10'}`}>
                                      ...
                                      <div className="text-right"><div className={`font-bold ${item.status === 'expired' ? 'text-red-400' : 'text-amber-400'}`}>{item.status === 'expired' ? `過期 ${Math.abs(item.days)}` : `剩 ${item.days}`}</div></div>
                                  </div>
                              ))}
                  ...
                  return (
                      <div className="hidden md:grid grid-cols-1 md:grid-cols-2 gap-4 flex-none">
                          <div className="bg-gradient-to-r from-slate-800 to-slate-700 rounded-lg p-4 text-white shadow-sm flex gap-4 relative overflow-hidden"><div className="w-1/3 border-r border-white/10 pr-2"><div className="font-bold mb-2 flex items-center text-xs"><Globe size={14} className="mr-1"/> 中港提醒</div><div className="text-2xl font-bold text-red-400">{cbAlerts.length}</div></div><AlertList items={cbAlerts} onItemClick={(item:any) => { setActiveTab('cross_border'); setActiveCbVehicleId(item.id); }} /></div>
                          <div className="bg-gradient-to-r from-blue-900 to-blue-800 rounded-lg p-4 text-white shadow-sm flex gap-4 relative overflow-hidden"><div className="w-1/3 border-r border-white/10 pr-2"><div className="font-bold mb-2 flex items-center text-xs"><Database size={14} className="mr-1"/> 文件提醒</div><div className="text-2xl font-bold text-amber-400">{docAlerts.length}</div></div><AlertList items={docAlerts} onItemClick={(item:any) => { setActiveTab('database'); setEditingEntry(item.raw); setIsDbEditing(true); }} /></div>
                      </div>
                  );
              })()}
                          ...
                          <table className="w-full text-left text-sm whitespace-nowrap relative">
                            <thead className="sticky top-0 bg-slate-100 z-10 shadow-sm text-slate-600 font-bold text-xs uppercase">
                                <tr className="border-b">
                                    ...
                                    <th className="p-3 hidden md:table-cell">詳細規格</th> {/* 手機隱藏 */}
                                  ...
                                  return (
                                    <tr key={car.id} className="border-b hover:bg-blue-50 cursor-pointer transition-colors group text-xs md:text-sm" onClick={() => setEditingVehicle(car)}>
                                      <td className="p-3 text-center" onClick={e=>e.stopPropagation()}>
                                          <button onClick={() => setShareVehicle(car)} className="text-slate-400 hover:text-blue-600 p-1"><Share2 size={16}/></button>
                                      </td>
                                      ...
                                      <td className="p-3">
                                          ...
                                          {/* 手機版顯示詳細規格 */}
                                          <div className="md:hidden text-[10px] text-gray-400 mt-1 space-x-1">
                                              <span>{car.previousOwners}手</span>|<span>{car.seating}座</span>|<span>{car.engineSize}cc</span>
                                          </div>
                                      </td>
                                      ...
                                      <td className="p-3 text-right">
                                          <div className="font-bold text-slate-700">{formatCurrency(car.price)}</div>
                                          <div className="text-[10px] text-gray-400 mt-1">牌: {car.licenseExpiry || '-'}</div>
                                      </td>
