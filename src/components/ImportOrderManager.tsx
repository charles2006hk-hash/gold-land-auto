...
// ★ 修復版 formatNum：強制轉換為字串並處理 undefined，防止 replace 報錯
...
export default function ImportOrderManager({ db, staffId, appId, settings, updateSettings }: any) {
        ...
        reader.onload = async (event) => {
            try {
                ...
                if (Array.isArray(rawData)) {
                    ...
                    if (coreData.history) {
                        list = Array.isArray(coreData.history) ? coreData.history : Object.keys(coreData.history).map(k => ({ id: k, ...coreData.history[k] }));
                    } else if (coreData.import_orders) {
                        list = Array.isArray(coreData.import_orders) ? coreData.import_orders : Object.keys(coreData.import_orders).map(k => ({ id: k, ...coreData.import_orders[k] }));
                ...
                if (confirm(`準備搬遷 ${list.length} 筆數據至雲端？`)) {
    ...
    const handleSave = async () => {
        ...
        const record = {
            ...
            photos: orderPhotos.filter(p => p !== "loading..."), // 過濾掉未上傳完成的標籤
            status: editingId ? (history.find(h=>h.id===editingId)?.status || 'QUOTING') : 'QUOTING', createdBy: staffId
        };
        try {
            if (editingId) await updateDoc(doc(db, `artifacts/${appId}/staff/CHARLES_data/import_orders`, editingId), record);
            else await addDoc(collection(db, `artifacts/${appId}/staff/CHARLES_data/import_orders`), { ...record, timestamp: serverTimestamp() });
            alert("✅ 儲存成功！"); setView('history'); setOrderPhotos([]);
        } catch (e) { alert("儲存失敗"); }
    };
    ...
    const handleDelete = async (item: any) => {
        ...
        if (confirm("確定刪除此紀錄？")) await deleteDoc(doc(db, `artifacts/${appId}/staff/CHARLES_data/import_orders`, item.id));
    };
    ...
    return (
        <div className="bg-white md:bg-slate-100 h-full rounded-2xl shadow-sm border border-slate-200 flex flex-col overflow-hidden relative">
            ...
            <div className="bg-slate-900 text-white p-3 flex justify-between items-center z-30 flex-none safe-area-top">
                <div className="flex items-center gap-2"><div className="bg-blue-600 p-1.5 rounded-lg shadow-lg"><Ship size={18}/></div><span className="font-black text-sm md:text-lg tracking-tighter">海外訂車管家</span></div>
                    ...
                    <div className="w-full h-full flex flex-col bg-slate-50/50 overflow-hidden">
                        <div className="p-4 bg-white border-b border-slate-200 flex flex-col sm:flex-row gap-4 justify-between items-center shadow-sm z-10 flex-none">
                            <div className="flex gap-2 overflow-x-auto pb-1 sm:pb-0 scrollbar-hide w-full sm:w-auto">
                                ...
                                {Object.entries(STATUS_OPTIONS).map(([k,v]:any) => (
                                    <button key={k} onClick={() => setFilterStatus(k)} className={`px-4 py-1.5 rounded-full text-xs font-bold border-2 transition whitespace-nowrap ${filterStatus === k ? v.color : 'bg-white text-slate-500 border-slate-200'}`}>{v.label}</button>
                                ))}
                            </div>
                            <div className="relative w-full sm:w-64"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input className="w-full pl-9 pr-4 py-2 rounded-lg border border-slate-300 focus:border-blue-500 outline-none font-bold text-xs" placeholder="搜尋型號或車身號碼..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} /></div>
                        </div>
                        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-4 max-w-6xl mx-auto w-full">
                            {filteredHistory.map(item => {
                                ...
                                return (
                                    <div key={item.id} className={`p-5 rounded-2xl shadow-sm border border-slate-200 hover:shadow-md transition-all border-l-8 ${item.isLocked ? 'border-l-yellow-400' : (item.status === 'IN_PROGRESS' ? 'border-l-orange-400' : (item.status === 'DELIVERED' ? 'border-l-emerald-400' : 'border-l-slate-400'))} bg-white flex flex-col`}>
                                        <div className="flex justify-between items-start mb-3">
                                            <div className="flex-1 flex gap-4">
                                                <div className="w-20 h-16 rounded-lg bg-slate-100 border border-slate-200 flex-none overflow-hidden flex items-center justify-center">{item.photos?.length > 0 ? <img src={item.photos[0]} className="w-full h-full object-cover" /> : <ImageIcon size={20} className="text-slate-300"/>}</div>
                                                <div className="flex-1 min-w-0">
                                                    <div className="flex items-center gap-2 mb-1.5"><span className={`text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider border ${st.color}`}>{st.label}</span><span className="bg-blue-100 text-blue-900 text-[10px] font-black px-2 py-0.5 rounded uppercase tracking-wider border border-blue-200">{item.region}</span><span className="text-[10px] text-slate-400 font-bold">{item.date}</span></div>
                                                    <div className="font-black text-slate-900 text-xl tracking-tight leading-tight truncate">{item.details?.manufacturer || item.carInfo?.make} {item.details?.model || item.carInfo?.model} <span className="font-bold text-slate-500 text-lg">{item.details?.year || item.carInfo?.year}</span></div>
                                            ...
                                            <div className="flex flex-col items-end gap-2 ml-4">
                                                <div className="flex gap-1.5 flex-wrap justify-end">
                                                    <button onClick={() => handleImportToInventory(item)} className={`p-1.5 rounded-lg transition flex items-center gap-1 font-bold text-[10px] border shadow-sm ${item.isImported ? 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50' : 'bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100'}`} title={item.isImported ? "取消匯入" : "匯入至車輛庫存"}>{item.isImported ? <RotateCcw className="w-4 h-4"/> : <Database className="w-4 h-4"/>} <span className="hidden sm:inline">{item.isImported ? '取消匯入' : '匯入主庫存'}</span></button>
                                                    <button onClick={() => setPreviewItem(item)} className="p-1.5 text-blue-600 bg-blue-50 hover:bg-blue-100 rounded-lg transition" title="預覽"><Eye className="w-4 h-4"/></button>
                                        ...
                                        <div className="mt-2"><TransportProgressBar departureDate={item.details?.departureDate} durationDays={item.details?.shippingDuration} type={item.details?.transportType} /></div>
                                        <div className="flex justify-between items-end border-t border-slate-100 pt-3 mt-1">
                                            <div className="text-[10px] text-slate-500 font-bold space-y-0.5"><div>到港: <span className="text-slate-800">{fmt(item.results?.landedCost)}</span></div><div>A1稅: <span className="text-slate-800">{fmt(item.results?.frtTax)}</span></div></div>
