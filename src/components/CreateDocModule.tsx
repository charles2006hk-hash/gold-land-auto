                    ...
                    <div className="p-10 font-sans text-slate-900 h-full flex flex-col relative">
                        ...
                        <div className="flex-1">
                            {(!isBill) ? (
                                <>
                                        ...
                                        <div className="mb-3">
                                            <div className="bg-slate-100 border border-slate-200 rounded-lg p-1.5 flex gap-2 justify-center items-center">
                                                {formData.contractPhotos.map((url: string, idx: number) => (
                                                    <div key={idx} className="w-[36mm] h-[25mm] rounded-md overflow-hidden border border-slate-300 bg-white shadow-sm flex-shrink-0"><img src={url} className="w-full h-full object-cover" alt="car-thumb" /></div>
                                                ))}
                                            </div>
                                        </div>
                                    )}
                                                ...
                                                <tbody>
                                                    ...
                                                    {formData.orderType === 'Overseas' && (
                                                        <>
                                                            ...
                                                            <tr>
                                                                <td colSpan={3} className="border p-1.5 text-slate-600"><span className="font-bold text-slate-800">Included (包含項目):</span> {' '} {overseasOptions.filter(opt => (formData as any)[opt.k]).map(opt => opt.l).join(', ') || 'N/A'}</td>
                                                            ...
                                                            <tr>
                                                                <td colSpan={3} className="border p-1.5 text-slate-600"><span className="font-bold text-slate-800">Included (包含項目):</span> {' '} {localOptions.filter(opt => (formData as any)[opt.k]).map(opt => opt.l).join(', ') || 'N/A'}</td>
                                    ...
                                    <div className="mb-3 break-inside-avoid">
                                        ...
                                        <table className="w-full text-[10px] border-collapse border border-slate-300">
                                            <tbody>
                                                <tr><td className="border p-1.5 font-bold w-1/2">{formData.orderType === 'Overseas' ? 'Overseas & Local Charges (海外與本地總費用)' : 'Vehicle Price (車價)'}</td><td className="border p-1.5 text-right font-mono font-bold">{formatCurrency(basePrice)}</td></tr>
                                                {docItems.filter((i: any) => i.isSelected).map((item: any, i: number) => (
                                                    <tr key={i} className="border-b"><td className="border p-1.5 text-slate-600 pl-4">+ {item.desc} {item.isFree ? <span className="font-bold text-slate-400">(贈送 F.O.C.)</span> : ''}</td><td className="border p-1.5 text-right font-mono">{item.isFree ? 'F.O.C.' : formatCurrency(item.amount)}</td></tr>
                                                ))}
                                                ...
                                                <tr className="bg-red-50/50 font-black"><td className="border p-1.5 uppercase text-[11px]">Balance Due (總結餘/尾數)</td><td className="border p-1.5 text-right font-mono text-[13px] text-red-600">{formatCurrency(balance)}</td></tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </>
                                ...
                                <table className="w-full text-[10px] border-collapse mb-6">
                                    ...
                                    <tbody>
                                        <tr className="border-b"><td className="p-2 font-bold text-purple-800">{formData.orderType === 'Overseas' ? 'Overseas & Local Charges (海外與本地總費用)' : `Vehicle Price (${formData.make} ${formData.model})`}</td><td className="p-2 text-right font-mono font-bold text-purple-800">{formatCurrency(basePrice)}</td></tr>
                                        {docItems.filter((i: any) => i.isSelected).map((item: any, i: number) => (
                                            <tr key={i} className="border-b"><td className="p-2 font-medium text-slate-600 pl-4">+ {item.desc} {item.isFree ? <span className="font-bold text-slate-400">(贈送 F.O.C.)</span> : ''}</td><td className="p-2 text-right font-mono">{item.isFree ? 'F.O.C.' : formatCurrency(item.amount)}</td></tr>
                                        ))}
                                        {depositItems.map((item: any, idx: number) => (
                            ...
                            {selectedDocType === 'receipt' && (
                                <div className="mb-4 p-2 bg-slate-50 border border-slate-200 rounded text-[10px]"><span className="font-bold">Payment Method:</span> {formData.paymentMethod}</div>
                            )}
                            ...
                            {formData.remarks && (
                                <div className="mb-4 border border-dashed border-slate-300 p-2 bg-slate-50 rounded break-inside-avoid">
                                    <p className="text-[9px] font-bold text-slate-500 mb-1">Remarks:</p><p className="text-[9px] whitespace-pre-wrap font-mono leading-tight">{formData.remarks}</p>
                                </div>
                            )}
                        </div>
                                ...
                                <div className="space-y-2 mb-3">
                                    {docItems.map((item: any) => (
                                        <div key={item.id} className="flex items-center text-xs bg-white p-2 rounded-lg border border-green-100 shadow-sm transition-all">
                                            ...
                                            <input value={item.desc} onChange={(e) => { const newDesc = e.target.value; setDocItems((prev: any[]) => prev.map((i: any) => i.id === item.id ? { ...i, desc: newDesc } : i)); }} className={`flex-1 bg-transparent outline-none font-bold focus:border-b border-blue-300 mr-2 ${item.isFree ? 'text-slate-400 line-through' : 'text-slate-700'}`} />
