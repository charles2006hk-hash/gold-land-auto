...
export default function CreateDocModule({ 
    ...
    const toggleItem = (id: string) => {
        setDocItems((prev: any[]) => prev.map((item: any) => item.id === id ? { ...item, isSelected: !item.isSelected } : item));
    };

    const deleteItem = (id: string) => {
        setDocItems((prev: any[]) => prev.filter((item: any) => item.id !== id));
    };
                                            ...
                                            <table className="w-full text-[10px] border-collapse border border-slate-300">
                                                <tbody>
                                                    ...
                                                    {formData.orderType === 'Overseas' && (
                                                        <>
                                                            <tr>
                                                                <td colSpan={4} className="border p-1.5 bg-slate-100 font-bold text-center">Estimated Overseas Charges (預估當地費用)</td>
                                                            </tr>
                                                            <tr>
                                                            ...
                                                            <tr>
                                                                <td colSpan={4} className="border p-1.5 bg-slate-100 font-bold text-center">Estimated Local Charges (預估到港本地費用)</td>
                                                            </tr>
                                                            <tr>
                                                                <td colSpan={3} className="border p-1.5 text-slate-600">
                                                                    <span className="font-bold text-slate-800">Included (包含項目):</span> {' '}
