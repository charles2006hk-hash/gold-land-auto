                        ...
                        <div className="space-y-3">
                                ...
                                <div className="space-y-2 mb-3">
                                    {depositItems.map((item, idx) => (
                                        <div key={item.id} className="flex items-center gap-2 bg-white p-1.5 rounded-lg border border-yellow-100 shadow-sm">
                                            ...
                                            <input type="number" value={item.amount} onChange={(e) => { const newArr = [...depositItems]; newArr[idx].amount = Number(e.target.value); setDepositItems(newArr); }} className="w-24 bg-transparent rounded px-1 text-right text-sm text-blue-600 font-black font-mono outline-none focus:bg-yellow-50" />
                                            <button onClick={() => setDepositItems(prev => prev.filter(i => i.id !== item.id))} className="text-yellow-400 hover:text-white hover:bg-red-500 p-1.5 rounded-md transition-colors"><X size={14}/></button>
                                        </div>
                                    ))}
                                </div>

                                <div className="text-right mb-3">
                                    <button onClick={() => setDepositItems([...depositItems, { id: Date.now().toString(), label: selectedDocType === 'receipt' ? 'Payment (本次收款)' : 'Deposit (訂金)', amount: 0 }])} className="text-[10px] font-bold bg-white text-yellow-700 px-3 py-1.5 rounded-lg border border-yellow-200 hover:bg-yellow-100 shadow-sm active:scale-95 transition-transform">
                                ...
                                <div className="space-y-2 mb-3">
                                    {docItems.map((item) => (
                                        <div key={item.id} className="flex items-center text-xs bg-white p-2 rounded-lg border border-green-100 shadow-sm transition-all">
                                            ...
                                            <button onClick={() => deleteItem(item.id)} className="text-gray-400 hover:text-white hover:bg-red-500 p-1.5 rounded-md ml-2 transition-colors"><X size={14}/></button>
                                        </div>
                                    ))}
                            ...
                            <div className="p-3 bg-white rounded-xl border border-slate-300 shadow-sm">
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-3 bg-slate-50 p-2 rounded-lg border border-slate-100">
                                    <label className="flex items-center text-xs font-bold text-slate-700 cursor-pointer"><input type="checkbox" checked={checklist.vrd} onChange={e=>setChecklist({...checklist, vrd: e.target.checked})} className="mr-2 accent-blue-600 w-4 h-4"/> 牌薄 (VRD)</label>
                                    <label className="flex items-center text-xs font-bold text-slate-700 cursor-pointer"><input type="checkbox" checked={checklist.keys} onChange={e=>setChecklist({...checklist, keys: e.target.checked})} className="mr-2 accent-blue-600 w-4 h-4"/> 後備匙</label>
                                    <label className="flex items-center text-xs font-bold text-slate-700 cursor-pointer"><input type="checkbox" checked={checklist.tools} onChange={e=>setChecklist({...checklist, tools: e.target.checked})} className="mr-2 accent-blue-600 w-4 h-4"/> 工具</label>
                                    <label className="flex items-center text-xs font-bold text-slate-700 cursor-pointer"><input type="checkbox" checked={checklist.manual} onChange={e=>setChecklist({...checklist, manual: e.target.checked})} className="mr-2 accent-blue-600 w-4 h-4"/> 說明書</label>
                                </div>
