...
export default function DocumentTemplate({ previewDoc, selectedVehicle, docType, COMPANY_INFO }: any) {
    ...
    // 如果是海外訂單，基礎車價直接等於海外+本地雜費總和，不再疊加原本的 price
    ...
    if (activeType === 'purchase_contract') {
        ...
        docTitleEn = "VEHICLE CONSIGNMENT AGREEMENT"; 
        docTitleCh = "汽車寄賣合約"; 
        isConsignment = true;
    ...
    const HeaderSection = () => (
        <div className="flex justify-between items-start mb-4 border-b-2 border-slate-800 pb-2">
            <div className="flex items-center gap-3">
                ...
                />
                <div>
                    <h1 className="text-lg font-black text-slate-900 tracking-wide uppercase leading-none">
                        {companyEn}
                    </h1>
                    <h2 className="text-md font-bold text-slate-700 tracking-widest leading-tight mt-1">
                        {COMPANY_INFO.name_ch}
                    </h2>
            ...
            <div className="text-right">
                <div className="text-md font-black text-slate-800 uppercase tracking-widest border-b-2 border-slate-800 inline-block mb-1">
                    {docTitleEn}
                </div>
                <div className="text-[10px] font-bold text-slate-600 tracking-[0.3em] text-center">
                    {docTitleCh}
                </div>
                <div className="mt-1 text-[9px] font-mono">
                    NO: {activeType.slice(0,3).toUpperCase()}-{today.replace(/\//g,'')}-{displayId}
                </div>
                <div className="text-[9px] font-mono font-bold text-blue-800 uppercase">
                    DATE: {today}
                </div>
            </div>
        </div>
    ...
    const AttachmentsSection = () => (
        <div className="mb-3 border border-slate-300 p-2 text-[10px] bg-slate-50 no-break">
            <div className="font-bold mb-1 uppercase border-b border-slate-300 pb-1">
                Attachments (隨車附件):
            </div>
            <div className="flex flex-wrap gap-4">
                <div className="flex items-center">
                    <div className="w-3 h-3 border border-black mr-1 flex items-center justify-center">
                        {checklist.vrd && <Check size={10}/>}
                    </div> 
                    VRD (牌薄)
                </div>
                <div className="flex items-center">
                    <div className="w-3 h-3 border border-black mr-1 flex items-center justify-center">
                        {checklist.keys && <Check size={10}/>}
                    </div> 
                    Spare Key (後備匙)
                </div>
                <div className="flex items-center">
                    <div className="w-3 h-3 border border-black mr-1 flex items-center justify-center">
                        {checklist.tools && <Check size={10}/>}
                    </div> 
                    Tools (工具)
                </div>
                <div className="flex items-center">
                    <div className="w-3 h-3 border border-black mr-1 flex items-center justify-center">
                        {checklist.manual && <Check size={10}/>}
                    </div> 
                    Manual (說明書)
                </div>
                {checklist.other && (
                    <div className="flex items-center font-bold border-b border-black px-2">
                ...
                {showStampAndSig && (
                    <>
                        <div className="absolute -top-8 left-1/2 -translate-x-1/2 opacity-90">
                            <CompanyStamp nameEn={companyEn} nameCh={COMPANY_INFO.name_ch} />
                        </div>
    ...
    if (activeType.includes('contract') || isQuotation) {
        ...
        return (
            <div id="print-root" className="max-w-[210mm] mx-auto bg-white p-6 min-h-[297mm] text-slate-900 font-sans relative shadow-lg print:shadow-none print:w-full print:p-0">
                <PrintStyle />
                <HeaderSection />
                    ...
                    <div className="no-break">
                        ...
                        <div className="border border-slate-300 p-2 text-[9px] min-h-[50px]">
                            <p>
                                ...
                                <b className="text-xs font-mono ml-1">{activeVehicle.regMark || 'TBC'}</b>
                            </p>
                            <p>
                                <span className="text-slate-500 font-bold">MAKE/MODEL:</span> 
                                <span className="ml-1">{activeVehicle.make} {activeVehicle.model}</span>
                            </p>
                            <p>
                                <span className="text-slate-500 font-bold">CHASSIS:</span> 
                                <span className="font-mono ml-1">{activeVehicle.chassisNo || 'TBC'}</span>
                            </p>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="mb-3 no-break">
                        <div className="bg-slate-100 border border-slate-200 rounded-lg p-1.5 flex gap-2 justify-center items-center">
                            {carPhotos.map((url: string, idx: number) => (
                                <div key={idx} className="w-[36mm] h-[25mm] rounded-md overflow-hidden border border-slate-300 bg-white shadow-sm">
